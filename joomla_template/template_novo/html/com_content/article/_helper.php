...
class TemplateContentArticleHelper {
	...
	static function customImg( $artigo = '' )
	{
		preg_match('/<img(.*?)>/', $artigo, $matches);
		...
		if($img != ""){
			...
			return $output;
		 	...
		 	return JURI::base().'images/banners/mecFace.jpg';	
		}
	}

	static function limitarTextoDesc( $artigo = '' , $nchars = 160, $complemento = '')
	{
		$intro = strip_tags($artigo);
		$intro = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x80-\x9F]/u', '', $intro);
		//180
		$pos = strpos($intro, ' ', $nchars );
		if( $pos !== false)
		{

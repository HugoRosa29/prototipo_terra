...
(function(){
  ...
  var searchIndex = [
    ...
    { title:'Nosso papel — a Terracap a serviço do DF', keywords:'missao institucional papel empresa publica sustentabilidade meio ambiente cerrado funcao social desenvolvimento urbano qualidade de vida quem somos', href:'#nosso-papel' },
    { title:'Indicadores institucionais', keywords:'indicadores numeros hectares regularizados familias atendidas empreendimentos estatisticas territorio em numeros', href:'#indicadores' },
...
/* ---------- menu lateral das internas: subitens em acordeão ---------- */
(function(){
  var sides = Array.prototype.slice.call(document.querySelectorAll('.interna-direita'));
  if(!sides.length) return;

  sides.forEach(function(side){
    var items = Array.prototype.slice.call(side.querySelectorAll('li'));
    items.forEach(function(li){
      var sub = null;
      for(var i = 0; i < li.children.length; i++){
        if(li.children[i].tagName === 'UL'){ sub = li.children[i]; break; }
      }
      if(!sub || !sub.children.length) return;

      li.classList.add('tem-sub');

      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'sub-toggle';
      btn.setAttribute('aria-expanded', 'false');
      btn.setAttribute('aria-label', 'Expandir subitens');
      btn.innerHTML = '<svg viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2.5 4l3.5 4 3.5-4"/></svg>';
      li.insertBefore(btn, sub);

      function set(open){
        li.classList.toggle('aberto', open);
        btn.setAttribute('aria-expanded', open ? 'true' : 'false');
        btn.setAttribute('aria-label', open ? 'Recolher subitens' : 'Expandir subitens');
      }

      btn.addEventListener('click', function(e){
        e.preventDefault();
        e.stopPropagation();
        set(!li.classList.contains('aberto'));
      });

      // seção da página atual começa aberta
      if(li.classList.contains('active') || li.classList.contains('current') || sub.querySelector('.active, .current')){
        set(true);
      }
    });
  });
